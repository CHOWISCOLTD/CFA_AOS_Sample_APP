...
public class CFALocalPores101 {
    ...
    public static String doAnalysis(Context context,
        ...
        // Pores ROIs, respective bounding Rectangles, and mask images.
        // --- ROI mask images.
        Mat frontPoresROI = Mat.zeros(new Size(originalWidth, originalHeight), CV_8UC1);

        // --- ROI for AI inference, cropped out of the full size original image.
        Mat frontPoresROICropped = new Mat();
        
        Rect frontPoresROIBoundRect = new Rect();
        
        Mat frontPoresMask = Mat.zeros(new Size(originalWidth, originalHeight), CV_8UC1);
        ...
        List<Integer> poresROIBoundShape = Collections.unmodifiableList(Arrays.asList(245, 233, 232, 231, 230, 229, 228, 117, 50, 205, 206, 203, 48, 115, 220, 45, 4, 275, 440, 344, 279, 423, 426, 425, 280, 346, 448, 449, 450, 451, 452, 453, 465));

        int x = 0, y = 0;
        List<Point> poresROIPoints = new ArrayList<>();
        for(Integer index : poresROIBoundShape) {
            x = frontCoordinates.get(index).get(0).intValue();
            y = frontCoordinates.get(index).get(1).intValue();
            poresROIPoints.add(new Point(x, y));
        }
        MatOfPoint poresROIPointMat = new MatOfPoint();
        poresROIPointMat.fromList(poresROIPoints);
        List<MatOfPoint> poresROIContour = new ArrayList<>();
        poresROIContour.add(poresROIPointMat);
        

        // Get pores ROI in front image
        drawContours(frontPoresROI, poresROIContour, -1, new Scalar(255), FILLED);
        frontPoresROIBoundRect = boundingRect(frontPoresROI);
        //rectangle(frontPoresROI, frontPoresROIBoundRect, new Scalar(255), 10);
        cvtColor(frontPoresROI, frontPoresROI, COLOR_GRAY2BGR);
        bitwise_and(frontPoresROI, frontOriginalImg, frontPoresROI);
        frontPoresROICropped = new Mat(frontPoresROI, frontPoresROIBoundRect);
        
        // For later indexing
        cvtColor(frontPoresROI, frontPoresROI, COLOR_BGR2GRAY);

        MyUtil.saveMatToGallery(context, "front pores ROI.jpg", "front pores ROI cutout", frontPoresROICropped);
        
        // ----------------------------------- 1. AI detections ------------------------------
        // Load AI Model
        Interpreter poresAIModel = new Interpreter(poresTFModelFile);

        int roiHeight = 0;
        int roiWidth = 0;
        roiHeight = frontPoresROICropped.height();
        roiWidth = frontPoresROICropped.width();
        Mat frontPoresMaskCropped = aiInference(context, poresAIModel, frontPoresROICropped, roiHeight, roiWidth);

        // Process mask images for front face wrinkles.
        //
        // Front under eye wrinkles.
        Mat subMask = new Mat();
        subMask = frontPoresMask.submat(frontPoresROIBoundRect);
        frontPoresMaskCropped.copyTo(subMask);

        //MyUtil.saveMatToGallery(context, "front pores.jpg", "front pores", frontPoresMask);

        // ------------------------------------------ 3. Indexing ------------------------------------------
        //
        // Pores scoring.

        double frontPoresRaw = 0, frontPoresScore = 0;
        double frontFullFaceCount = countNonZero(frontFullFaceMask);
        
        if(frontFullFaceCount > 0) frontPoresRaw = (double) 1000 * countNonZero(frontPoresMask) / frontFullFaceCount;
        frontPoresScore = getCFAPoresLevel(frontPoresRaw, capturedWithFrontCamera);
        
        // ------------------------------------------ 4. Prepare Output ------------------------------------------
        //
        // ----- (a) ----- Save wrinkle mask images first.
        cvtColor(frontPoresMask, frontPoresMask, COLOR_GRAY2BGR);
        Imgcodecs.imwrite(frontPoresMaskOutputPath, frontPoresMask);
        
        // ----- (b) ------ Prepare input paths for mask images.
        String frontPoresMaskInputPath = frontPoresMaskOutputPath;
        ...
        double saveFrontRes = myCFAImgProc.CFAGetAnalyzedImgJni(frontPPLOriginalInputPath, frontPoresMaskInputPath, frontPoresResultOutputPath, alpha, maskB, maskG, maskR, contourB, contourG, contourR);

        // ---------- Prepare returned values.
        //
        String returnString = String.valueOf(frontPoresScore);
        
        System.out.println("Returned String for Pores is" + returnString);

        return returnString;
    }

    private static Mat aiInference(Context context, Interpreter poresModel, Mat originalImg, int originalHeight, int originalWidth){

        // ------------------------------------------ 1. AI Model  ------------------------------------------
        // Allocate input and output tensors.
        poresModel.allocateTensors();

        // Prepare input tensor from OpenCV image.
        int[] inputShape = poresModel.getInputTensor(0).shape(); // num, height, width, channel.
        ...
        Log.println(Log.VERBOSE, "TS pores input: ", String.valueOf(timestamp1End - timestamp1Start));
        ...
        poresModel.run(input, output);
        long timestamp2End = System.currentTimeMillis();
        Log.println(Log.VERBOSE, "TS pores inference: ", String.valueOf(timestamp2End - timestamp2Start));

        // Post processing.
        // Get our pores mask
        long timestamp3Start = System.currentTimeMillis();

        Mat stagedPoresMask = Mat.zeros(input_height, input_width, CV_8UC1);

        int backgroundClassID = 0;
        int poreClassID = 1;
        for (int y = 0; y < input_height; y++) {
            for (int x = 0; x < input_width; x++) {
                if(output[0][y][x][poreClassID] > 0.5) stagedPoresMask.put(y, x, 255);
            }
        }

        long timestamp3End = System.currentTimeMillis();
        Log.println(Log.VERBOSE, "TS pores output: ", String.valueOf(timestamp3End - timestamp3Start));
        ...
        Mat poresMask = new Mat(stagedPoresMask, maskForegroundRect);

        if (originalHeight * originalWidth < staging_height * staging_width) {
            resize(poresMask, poresMask, new Size(originalWidth, originalHeight), 0, 0, INTER_AREA);
        } else if (originalHeight * originalWidth > staging_height * staging_width) {
            resize(poresMask, poresMask, new Size(originalWidth, originalHeight), 0, 0, INTER_LINEAR);
        }

        // Restore detected features after image resizing. We can't retain all 255 values in resized image.
        threshold(poresMask, poresMask, 10, 255, THRESH_BINARY);

        return poresMask;
    }

    private static double getCFAPoresLevel(double pureValue, boolean capturedWithFrontCamera) {
